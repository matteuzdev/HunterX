...
import { Coins, CreditCard, Gift, ShieldCheck, Zap } from "lucide-react";
  ...
  return (
    <section className="space-y-5">
      <div>
        ...
        <p className="mt-2 text-sm text-slate-500">1 token = 1 empresa liberada em uma nova coleta. Resultados já salvos podem ser reabertos sem gastar novamente.</p>
      </div>

      <Card className="overflow-hidden bg-slate-950 p-6 text-white">
        <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
          <div>
            <span className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[.14em] text-amber-300"><Coins className="size-4" /> Saldo atual</span>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        {packages.map((pkg) => (
          <Card key={pkg.id} className="relative p-5">
            ...
            <p className="mt-1 text-xs text-slate-400">{pkg.tokens} base {pkg.bonusTokens ? `+ ${pkg.bonusTokens} bônus` : ""}</p>
            <div className="mt-5 text-2xl font-black">{money(pkg.priceCents, pkg.currency)}</div>
            {pkg.checkoutUrl ? (
              <a href={pkg.checkoutUrl} target="_blank" rel="noreferrer">
                <Button className="mt-5 w-full"><CreditCard className="size-4" /> Comprar tokens</Button>
              </a>
            ) : (
              <Button className="mt-5 w-full" variant="secondary" disabled title="Gateway de pagamento ainda não conectado">
                <CreditCard className="size-4" /> Checkout em configuração
              </Button>
            )}
          </Card>
        ))}
